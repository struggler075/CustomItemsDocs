...
Ниже версия вообще для новичка, без предположения, что ты умеешь работать с JSON.

Важно: в этой инструкции слово "блок" не про блок Minecraft. Здесь "блок предмета" означает просто кусок текста в файле, который начинается с `{` и заканчивается `}`.

### Сначала запомни 3 простые вещи

1. Весь файл `items.json` должен оставаться внутри внешних скобок:
...
3. Если предметов несколько, между ними нужна **запятая**:
...
- после первого предмета запятая **есть**;
- после последнего предмета запятой **нет**.

### Как выглядит файл, если предмет только один
...
### Как выглядит файл, если предметов уже два

Вот самый важный пример. Новый предмет добавляется **ниже первого**, а после первого ставится запятая:
...
```json
{
  "items": [
    ...
    {
      "id": "my_second_item",
      "displayName": "Мой второй предмет",
...
### Как добавить второй предмет пошагово

Допустим, у тебя уже есть один предмет и ты хочешь добавить ещё один.

1. Открой `items.json`.
2. Найди первый предмет. Он заканчивается вот так:

```json
    }
```

3. Если после него ты добавляешь ещё один предмет, преврати этот конец в:

```json
    },
```

То есть просто добавь запятую.

4. Сразу после этого вставь новый предмет:
...
5. Убедись, что после **последнего** предмета запятой нет.
6. Сохрани файл.

### Что нельзя удалять и ломать

В файле нельзя удалять эти части:
...
То есть новые предметы всегда вставляются **между**:

- `"items": [`
- `]`

### Готовый пример: было 1, стало 2
...
- после первого предмета появилась запятая `,`;
- второй предмет вставлен сразу под первым;
- в конце файла после последнего предмета запятой нет.
...
2. Проверь, что предмет загрузился:

```text
/customitems list
```

3. Выдай предмет себе:
...
```text
/customitems give Admin battle_cookie 1
```

Если предмет не появился, почти всегда причина в одной из трёх вещей:

- забыли запятую между предметами;
- поставили лишнюю запятую после последнего предмета;
- случайно удалили `]` или `}` внизу файла.
...
Если это звучит слишком сложно, запомни простое правило:

- новый предмет вставляется между `[` и `]`;
- если предмет не последний, после него нужна запятая;
- если предмет последний, запятая не нужна.
...
### Куда именно вставлять новый предмет

Смотри на такой каркас:
...
Если хочешь добавить второй предмет, делай так:
...
- у первого предмета в конце было `}`;
- стало `},`;
- ниже вставили второй предмет.
...
- между первым и вторым предметом нет запятой.
...
- после последнего предмета стоит лишняя запятая.
